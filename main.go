...
func main() {
	...
	s.AddTool(mcp.NewTool("click",
		...
		mcp.WithNumber("button", mcp.Description("Button to click (1: left, 2: middle, 3: right)")),
	), clickHandler(p))
	...
	s.AddTool(mcp.NewTool("press_key",
		mcp.WithDescription("Simulate pressing a specific key (with optional modifiers)"),
		mcp.WithString("key", mcp.Description("Key name (e.g., Enter, Escape, a, b, c)"), mcp.Required()),
...
func screenshotHandler(p *portal.Portal) server.ToolHandlerFunc {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		return mcp.NewToolResultImage("Screenshot captured", encoded, "image/png"), nil
	}
}
...
func clickHandler(p *portal.Portal) server.ToolHandlerFunc {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		if err := p.MovePointer(x, y); err != nil {
			return mcp.NewToolResultError(fmt.Sprintf("Failed to move pointer: %v", err)), nil
		}
		...
		if err := p.Click(button, 0); err != nil {
			return mcp.NewToolResultError(fmt.Sprintf("Failed to release button: %v", err)), nil
		}
	...
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		for _, r := range text {
			...
			if err := p.TypeKey(keysym, 0); err != nil {
				return mcp.NewToolResultError(fmt.Sprintf("Failed to release key: %v", err)), nil
			}
		}
...
func pressKeyHandler(p *portal.Portal) server.ToolHandlerFunc {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		if keySym == 0 {
			return mcp.NewToolResultError(fmt.Sprintf("Unknown key: %s", keyName)), nil
		}
